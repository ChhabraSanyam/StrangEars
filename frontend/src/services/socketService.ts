...
class SocketService {
  ...
  private setupEventListeners(): void {
    ...
    this.socket.on("error", (error: any) => {
      ...
      const errorMessage =
        error?.message || error?.toString() || "Unknown socket error";
    ...
    this.socket.on(
      "spam-warning",
      (data: { message: string; action: string; timeRemaining?: number }) => {
    ...
    if (profilePhoto && profilePhoto instanceof File) {
      ...
      reader.onload = () => {
        try {
          ...
          if (base64Photo.length > 200 * 1024) {
            // 200KB base64 limit (≈150KB file)
            console.warn(
              "Profile photo still too large after optimization, sending without photo"
            );
            ...
            this.eventHandlers.onError?.(
              "Profile photo optimization failed. Joining without photo."
            );
            return;
          }
          ...
          this.socket!.emit("join-session", data);
          this.eventHandlers.onError?.(
            "Failed to process profile photo. Joining session without photo."
          );
        }
      };
      ...
      reader.onerror = () => {
        ...
        this.socket!.emit("join-session", data);
        this.eventHandlers.onError?.(
          "Failed to read profile photo. Joining session without photo."
        );
      };
  ...
  isConnected(): boolean {
    return (
      this.connectionStatus === "connected" && this.socket?.connected === true
    );
  }
